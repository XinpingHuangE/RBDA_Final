...
def main():
    ...
    CITY_WISE_PER_HOUR_CRIME_RATE_QUERY = "SELECT city, bucket, location, sum(CAST(severity AS BIGINT))/count(severity) as crime_rate FROM {} WHERE city = '{}' AND from_unixtime(unix_timestamp(crime_date , 'MM/dd/yyyy'))>=from_unixtime(unix_timestamp('{}' ,'MM/dd/yyyy')) AND from_unixtime(unix_timestamp(crime_date , 'MM/dd/yyyy'))<=from_unixtime(unix_timestamp('{}', 'MM/dd/yyyy')) GROUP BY city, bucket, location".format(config.TABLE,CITY,START_DATE,END_DATE)
    CITY_WISE_PER_YEAR_CRIME_RATE_QUERY = "SELECT city, substr(crime_date, 7,10) as year, location, sum(CAST(severity AS BIGINT))/count(severity) as crime_rate  FROM {} WHERE city = '{}' GROUP BY city, substr(crime_date, 7,10), location".format(config.TABLE,CITY)
    # RANGE_QUERY_PER_CRIME = "SELECT city, location, sum(severity)/count(severity) FROM {} WHERE from_unixtime(unix_timestamp(crime_date , 'MM/dd/yyyy'))>=from_unixtime(unix_timestamp('{}' ,'MM/dd/yyyy')) AND from_unixtime(unix_timestamp(crime_date , 'MM/dd/yyyy'))<=from_unixtime(unix_timestamp('{}', 'MM/dd/yyyy')) GROUP BY city, location, category".format(config.TABLE,START_DATE,END_DATE)
    print("range query: ",CITY_WISE_PER_YEAR_CRIME_RATE_QUERY)

    results = runQuery(cursor,CITY_WISE_PER_YEAR_CRIME_RATE_QUERY)
