...
def main():
    ...
    RANGE_QUERY_PER_CRIME = "SELECT category, sum(severity)/count(severity) as crime_rate as avg_lon FROM {} WHERE from_unixtime(unix_timestamp(crime_date , 'MM/dd/yyyy'))>=from_unixtime(unix_timestamp('{}' ,'MM/dd/yyyy')) AND from_unixtime(unix_timestamp(crime_date , 'MM/dd/yyyy'))<=from_unixtime(unix_timestamp('{}', 'MM/dd/yyyy')) GROUP BY crime".format(config.TABLE,CITY,START_DATE,END_DATE)
